...
- *scheduling*: Opt-in cooperative scheduling of dataflow evaluation (`boolean` or `{signal?: AbortSignal}`). When enabled, long-running [runAsync](#view_runAsync) calls periodically yield to the browser event loop between transforms (via `scheduler.yield()` where available, otherwise a `setTimeout` fallback) so that large datasets do not block the main thread. Yields inherit the task priority of the caller; Vega never posts tasks with an explicit priority. If an `AbortSignal` is provided and aborted, the in-flight `runAsync` promise rejects with the signal's reason and the view should then be finalized. Calling [finalize](#view_finalize) also cancels any in-flight scheduled evaluation. While scheduling is enabled, synchronous [run](#view_run) calls are queued behind any in-flight evaluation rather than starting immediately. By default (`false`), evaluation runs synchronously exactly as before.
